...
export default function HomeScreen() {
  ...
  useEffect(() => {
    if (stats && stats.streak > 0) {
      Animated.loop( Animated.sequence([ Animated.timing(flameAnimation, { toValue: 1.2, duration: 800, useNativeDriver: true }), Animated.timing(flameAnimation, { toValue: 1, duration: 800, useNativeDriver: true }), ]) ).start();
    }
  }, [stats?.streak]);
        ...
        <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
          <View style={styles.header}>
            ...
            <View style={styles.streakBox}>
              <Animated.View style={{ transform: [{ scale: flameAnimation }] }}><Ionicons name="flame" style={getFlameStyle()} /></Animated.View>
              <Text style={[styles.streakText, { color: getFlameStyle().color }]}>{stats.streak}</Text>
            </View>
          </View>
          <View style={styles.card}><Text style={styles.cardTitle}>Seviye {currentLevel}</Text><View style={styles.barBack}><View style={[styles.barFront, { width: `${levelProgress}%` }]} /></View><Text style={styles.xpText}>{`${currentLevelXp}/${xpForNextLevel} XP`}</Text></View>
          <View style={styles.card}><Text style={styles.cardTitle}>Bu Hafta</Text><View style={styles.dayRow}>{(stats.weeklyLog || []).map((isCompleted, i) => ( <View key={i} style={[ styles.dayDot, isCompleted && { backgroundColor: Colors.primary, borderColor: Colors.primaryLight }]} /> ))}</View><Text style={styles.subText}>{(stats.weeklyLog || []).filter(Boolean).length}/7 gün tamamlandı</Text></View>
          <View style={styles.card}><Text style={styles.cardTitle}>Hızlı Başla</Text><Pressable style={styles.actionRow} onPress={() => navigation.navigate('TopicsTab')}><View style={[styles.iconCircle, { backgroundColor: Colors.primary + '20' }]}><Ionicons name="book" size={22} color={Colors.primary} /></View><Text style={styles.actionText}>Konu Çalış</Text><Ionicons name="chevron-forward" size={20} color={Colors.grey} /></Pressable><Pressable style={styles.actionRow} onPress={() => {}}><View style={[styles.iconCircle, { backgroundColor: Colors.warning + '20' }]}><Ionicons name="create" size={22} color={Colors.warning} /></View><Text style={styles.actionText}>10 Soruluk Test</Text><Ionicons name="chevron-forward" size={20} color={Colors.grey} /></Pressable></View>
        </ScrollView>
      </SafeAreaView>
    </GradientBg>
  );
